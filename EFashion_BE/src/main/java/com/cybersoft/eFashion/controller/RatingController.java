...
public class RatingController {
    ...
    public ResponseEntity<?> getAllRating(){
        ...
        responseData.setData(ratingServiceImp.getAllRating());
    ...
    public ResponseEntity<?> getRatingByProduct(@RequestParam int pro_id){
        ResponseData responseData = new ResponseData();
        responseData.setData(ratingServiceImp.getRatingByProduct(pro_id));
//        return new ResponseEntity<>(responseData, HttpStatus.OK);
        return new ResponseEntity<>(responseData, HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> getRatingByUser(@RequestParam int user_id){
        ResponseData responseData = new ResponseData();
        responseData.setData(ratingServiceImp.getRatingByUser(user_id));
        return new ResponseEntity<>(responseData, HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> addRating(@RequestParam("file") MultipartFile file,
        ...
        if(user_id == -1) {
            ...
            responseData.setData(ratingServiceImp.insertRating(file, rate_dto));
    ...
    public ResponseEntity<?>removeRating(@RequestParam int rating_id){
        ResponseData responseData = new ResponseData();
        responseData.setData(ratingServiceImp.removeRating(rating_id));
